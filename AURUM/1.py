...
out_dir = "/scratch/alice/b/bg205/DataCleaning_Aurum_v2"
out_path = os.path.join(out_dir, "filtered_diabetes_AURUM_codes.txt")

# ──────────────────────────────────────────────────────────
# 1. LOAD  (read everything as str first for safety)
# ──────────────────────────────────────────────────────────
df_codes = pd.read_csv(diabetes_code_doc, sep="\t", dtype=str, low_memory=False)

print("Loaded:", df_codes.shape)

# DBG: show actual columns
print(f"DBG| [COLUMNS] {list(df_codes.columns)}")

# Identify the medcode column dynamically
code_col = None
